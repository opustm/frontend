...
export default class Announcements extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      creationError: false,
      teamFilter: this.props.match.params.teamUsername
  ...
  async fetchData() {
    ...
    const eventRequest = await api.get(
      urls.event.fetchByUser(this.props.userInfo.id)
    );
    let teamEvents = eventRequest.data.filter(event => {
      return event.team;
    });
  ...
  async fetchAnnouncements() {
    const announcementRequest = await api.get(
      urls.announcement.fetchByUser(this.props.userInfo.id)
    );
    ...
    announcements.forEach(announcement => {
      let object = {
        ...
        event: announcement.event
          ? this.state.idToEventDict[announcement.event]['name']
          : 'N/A',
  ...
  async handleCreate(evt) {
    ...
    if (!this.createDataIsInvalid()) {
      ...
      let expirationDate = new Date(
        Date.now() + 3600000 * this.state.announcementDuration
      );
      ...
      let tableObject = {
        ...
        event: this.state.announcementEvent
          ? this.state.idToEventDict[this.state.announcementEvent]['name']
          : 'N/A',
      ...
      if (
        (tableObject.priority ===
          this.state.priorityDict[this.state.priorityFilter][0] ||
          this.state.priorityFilter === 0) &&
        (tableObject.team === this.state.teamFilter ||
          this.state.teamFilter === 'All')
      ) {
  ...
  createDataIsInvalid() {
    if (
      ...
      this.state.announcementTeam === -1 ||
      ![1, 2, 3].includes(this.state.announcementPriority) ||
      this.state.announcementDuration < 1
    ) {
  ...
  async deleteAnnouncement(announcementToDelete) {
    ...
    let filtered = this.state.userAnnouncements.filter(announcement => {
      return announcement !== announcementToDelete;
    });
    let filteredObjects = this.state.announcementObjects.filter(object => {
      return object.message !== announcementToDelete.announcement;
    });
    let filteredDisplayed = this.state.displayedAnnouncements.filter(object => {
      return object.message !== announcementToDelete.announcement;
    });
    this.setState({
  ...
  handlePriorityFilter(priority) {
    ...
    let filtered = this.state.announcementObjects.filter(object => {
      return (
        (object.priority === priorityName || priorityName === 'All') &&
        (object.team === this.state.teamFilter ||
          this.state.teamFilter === 'All')
      );
    });
    this.setState({
  ...
  render() {
    return (
      <Container fluid>
            ...
            >
              <Form.Group controlId="teamSelect">
                ...
                <Form.Control
                  ...
                  onChange={e => {
                    this.setState({
                      announcementTeam: parseInt(e.target.value)
                    });
                  }}
                >
              ...
              <Form.Group controlId="eventSelect">
                ...
                <Form.Control
                  ...
                  onChange={e => {
                    this.setState({
                      announcementEvent: parseInt(e.target.value)
                    });
                  }}
                >
                  <option selected disabled hidden>
                  ...
                  {this.state.announcementTeam ? (
                    this.state.teamEvents
                      .filter(event => {
              ...
              <Form.Group controlId="prioritySelect">
                ...
                <Form.Control
                  ...
                  onChange={e => {
                    this.setState({
                      announcementPriority: parseInt(e.target.value)
                    });
                  }}
                >
                  <option selected disabled hidden>
              ...
              <Form.Group controlId="durationInput">
                ...
                <Form.Control
                  ...
                  onChange={e => {
                    this.setState({
                      announcementDuration: parseInt(e.target.value)
                    });
                  }}
                />
              </Form.Group>
              <Form.Group controlId="announcementInput">
                ...
                <Form.Control
                  ...
                  onChange={e => {
                    this.setState({ announcementBody: e.target.value });
                  }}
                ></Form.Control>
              </Form.Group>
              <Alert variant="danger" hidden={!this.state.creationError}>
                Error: You've entered invalid data or forgotten to fill out one
                of the fields.
              </Alert>
        ...
        <Jumbotron>
          ...
          <Row>
            <Col>
              ...
              >
                <Icon.FiPlusCircle style={{ marginTop: '-3px' }} /> Create
                Announcement
              </Button>
            </Col>
            <Col>
            ...
            <Col>
              <Toast
                onClose={() => this.setState({ showCreatedToast: false })}
                show={this.state.showCreatedToast}
                delay={3000}
                autohide
              >
                <Toast.Header>
                ...
                <Toast.Body>
                  ...
                  {this.state.idToTeamDict[this.state.announcementTeam]}"
                  ...
                  {Object.keys(this.state.priorityDict).map(priorityKey => {
                    return (
                      <option key={priorityKey} value={priorityKey}>
                        {this.state.priorityDict[priorityKey][0]}
                      </option>
                    );
                  })}
                </FormControl>
              ...
              <Row style={{ marginRight: '10px' }}>
                ...
                >
                  ...
                  {this.state.userTeams.map(team => {
                    return (
                      <option
                        selected={team.name === this.state.teamFilter}
                        key={team.id}
                      >
                        {team.name}
                      </option>
        ...
        <BootstrapTable
          keyField="id"
          data={this.state.displayedAnnouncements}
          columns={columns}
          rowClasses={row => {
            return this.state.styleDict[row.priority];
          }}
        />
      </Container>
    );
